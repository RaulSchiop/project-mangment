...
if(Array.isArray(parsedStorage) && parsedStorage.length > 0){
 ...
 try{   const result= await fetch(`/api/deleteTasks?taskId=${id}`,{
      method:"DELETE"
    })
          ...
          <AnimatePresence>
            {tasks.length === 0 ? (
              ...
              tasks.map((task) => (
                ...
                <motion.li layout variants={itemsss} className={classes.list} key={task.id}>
                  <p style={{ color: 'red' }}>{task.status}</p>
                  <h4>{task.title}</h4>
                  <p style={{ color: '#007bff' }}>{formatDate(task.due_date)}</p>
                  <motion.button  whileHover={{ scale: 1.1 }}
      transition={{ type: "spring", stiffness: 400, damping: 10 }} className={classes.button} onClick={()=>handleDelete(task.id)}>Delete</motion.button>
