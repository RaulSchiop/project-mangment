...
const router=useRouter()
  ...
  async function fetchTasks(projectId, userId) {
    try {
      const response = await fetch(`/api/tasksFromProject?projectId=${projectId}&userId=${userId}`);
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }
      const data = await response.json();
...
function checkAuth(){
  ...
  return (
    <>
      <motion.ul initial="hidden" animate="visible" variants={container} className={classes.ulClass}>
      ...
      <Modal show={modalOpen} onClose={handleClose}>
        <motion.ul initial="hidden" animate="visible" variants={container} className={classes.ulWraper} >
          {
          tasks.length<1? <p className={classes.notasks}>No tasks</p>
:          
          tasks.map((task) => (
            <motion.li layout variants={itemsss} className={classes.ulClassModal} key={task.id}>
              <p style={{ color: 'red' }}>{task.status}</p>
              <h4>{task.title}</h4>
              <p style={{ color: '#007bff' }}>{formatDate(task.due_date)}</p>
            </motion.li>
          ))}
        </motion.ul>
      </Modal>
