  ...
  useEffect(() => {
    ...
    if (userid && date) {
      async function fetchTaskToday(useridd, datee) {
        try {
          const result = await fetch(`/api/todayTasks?userId=${useridd}&date=${datee}`, {
            headers: {
...
<button onClick={openModal} className={classes.buttonShortNotif}> <p> View all </p> </button> 
</motion.div>
  ...
  (task.map((item)=>(
      <li key={item.id}>
        <div className={classes.notifTask}>
          ...
          <h3 className={classes.blue}>{formatDate(item.due_date)}</h3>
          </div>
      </li>
    ))):<p>No tasks today</p>}
   </ul>
...
<motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.3}} className={classes.notifTaskModal}>
