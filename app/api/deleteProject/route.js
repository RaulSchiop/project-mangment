...
export async function DELETE(req) {
   ...
   if (!projectId) {
      return new NextResponse(JSON.stringify({ error: "no project id" }), {
         status: 400,
         headers: { "Content-Type": "application/json" },
      });
   }
   try {
      await db.query(`DELETE FROM ProjectMembers Where project_id=?`, [
         projectId,
      ]);
      ...
      if (row.affectedRows === 0) {
         return new NextResponse(
            JSON.stringify({ error: "projcet not found" }),
            {
               status: 404,
               headers: { "Content-Type": "application/json" },
            }
